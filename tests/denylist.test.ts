...
const BLOCKED = [
  ...
  'server.pem', 'private.key', 'cert.p12', 'bundle.pfx', 'app.jks', 'release.keystore', 'login.ppk',
  '.netrc', '.npmrc', '.pypirc', '.git-credentials', '.htpasswd', '.pgpass', '.my.cnf',
  '.bash_history', '.zsh_history', '.psql_history', '.node_repl_history',
  'terraform.tfvars', 'secrets.auto.tfvars',
  'credentials', 'credentials.json', 'client_secret_12345.json',
  'kubeconfig',
];

const ALLOWED = [
  'environment.ts', 'envelope.md', 'env.md', 'dotenv.ts',
  'id_rsa.pub', 'turkey.md', 'monkey.png', 'keynote.pdf',
  'history.md', 'npmrc-docs.md', 'report.json', 'config.yaml',
  'terraform.tf', 'credentials-guide.md',
];
