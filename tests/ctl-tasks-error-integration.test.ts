...
test('/tasks <agent> with no match but OTHER live agents running → error lists their names', async () => {
  const cfgDir = makeCfgDir('tgctl-tasks-nomatch-populated-');
  const bin = join(cfgDir, 'bin');
  mkdirSync(bin);
  // One live pane named "rig" — a selector for a DIFFERENT, unrelated name must
  // still report noMatch, but the message should list "rig" (not "running: none").
...
exit 0
`;
  writeFileSync(join(bin, 'tmux'), tmuxScript, { mode: 0o755 });
  const psScript = `#!/bin/sh
printf '    1     0 /sbin/launchd\\n'
printf ' 5001     1 claude --resume\\n'
exit 0
`;
  writeFileSync(join(bin, 'ps'), psScript, { mode: 0o755 });

  const { server, richMessages, sentMessages } = mockUpdatesAndCapture('/tasks totallyunrelated');
  try {
    const logFd = openSync(join(cfgDir, 'daemon.log'), 'a');
    await spawnDaemon(reg, {
      tgCtlPath: TG_CTL,
    ...
    expect(sentMessages).toHaveLength(1);
    expect(sentMessages[0].text as string).toMatch(/no agent matching ['"]?totallyunrelated['"]?/i);
    expect(sentMessages[0].text as string).toMatch(/running: rig/i);
    expect(richMessages).toHaveLength(0);
  } finally {
    server.stop(true);
  }
...
test('/tasks <agent> with an AMBIGUOUS (tied) match → distinct error, never "no agent matching"', async () => {
  const cfgDir = makeCfgDir('tgctl-tasks-ambiguous-');
  const bin = join(cfgDir, 'bin');
  mkdirSync(bin);
  // Two live panes with the SAME window name "claude" in different projects —
  // matchWindows ties both at the best score, so confident is false with >1
  // match (review round 5: this must NOT be reported as "no agent matching",
  // since an agent DID match — just not uniquely).
  const tmuxScript = `#!/bin/sh
case "$*" in
  *list-panes*)
    printf '4\\t0\\t%%5001\\t5001\\tnode\\tclaude\\t/Users/u/proj-a\\n'
    printf '7\\t0\\t%%5002\\t5002\\tnode\\tclaude\\t/Users/u/proj-b\\n'
    ;;
  *) exit 0 ;;
esac
exit 0
...
`;
  ...
  const { server, richMessages, sentMessages } = mockUpdatesAndCapture('/tasks claude');
  try {
    const logFd = openSync(join(cfgDir, 'daemon.log'), 'a');
    await spawnDaemon(reg, {
      tgCtlPath: TG_CTL,
    ...
    expect(sentMessages).toHaveLength(1);
    expect(sentMessages[0].text as string).toMatch(/matches multiple agents/i);
    expect(sentMessages[0].text as string).not.toMatch(/no agent matching/i);
    expect(richMessages).toHaveLength(0);
    expect(existsSync(taskMarker)).toBe(false);
    expect(existsSync(ghMarker)).toBe(false);
  } finally {
