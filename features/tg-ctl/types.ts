...
export type Action =
  ...
  // /next <ticket-id> — entrypoint composes from pm-cli's `pm why <id> --json`
  // (ticket lifecycle state), a live git status of the ticket's resolved
  // project pane, and a gh pr list CI/review rollup, sends a rich-HTML card.
  // Computed fresh on every query — no local state file (see next-command.ts).
  | { kind: 'next'; ticketId: string; replyToMessageId: number | null; threadId?: number | null }
